...
from lattice_su3.update import UpdateStats, heatbath_sweep, metropolis_sweep
...
def thermalize_heatbath(
    ...
    """Run heatbath thermalization sweeps in place.
    ...
    history = []
    for _ in range(sweeps):
        history.append(heatbath_sweep(links, geometry, beta, rng))
    return history


def thermalize_cold_start_heatbath(
    geometry: LatticeGeometry,
    beta: float,
    sweeps: int,
    rng: np.random.Generator | None = None,
) -> tuple[np.ndarray, list[UpdateStats]]:
    """Create and heatbath-thermalize a cold-start gauge configuration.
    ...
    """
    links = cold_start(geometry)
    history = thermalize_heatbath(links, geometry, beta, sweeps, rng)
    return links, history


def thermalize_hot_start_heatbath(
    geometry: LatticeGeometry,
    beta: float,
    sweeps: int,
    rng: np.random.Generator | None = None,
) -> tuple[np.ndarray, list[UpdateStats]]:
    """Create and heatbath-thermalize a hot-start gauge configuration.
